// KeyboardEvent.code -> PC/XT Set 1 make code。
//
// 采用 code（物理键位，与键盘布局/输入法无关）而非 key，因为 RDP scancode
// 本质就是物理位置。扩展键统一编码成 0xE0xx，后端 Scancode::from_u16 会识别
// 0xE000 位并置 extended 标志；普通键为单字节 make code。
//
// 未收录的键返回 undefined，调用方据此跳过发送（避免发出 code=0 的无效扫描码）。

const SCANCODES: Record<string, number> = {
  // 功能与控制
  Escape: 0x01,
  Backspace: 0x0e,
  Tab: 0x0f,
  Enter: 0x1c,
  Space: 0x39,
  CapsLock: 0x3a,
  // 字母
  KeyQ: 0x10, KeyW: 0x11, KeyE: 0x12, KeyR: 0x13, KeyT: 0x14, KeyY: 0x15,
  KeyU: 0x16, KeyI: 0x17, KeyO: 0x18, KeyP: 0x19,
  KeyA: 0x1e, KeyS: 0x1f, KeyD: 0x20, KeyF: 0x21, KeyG: 0x22, KeyH: 0x23,
  KeyJ: 0x24, KeyK: 0x25, KeyL: 0x26,
  KeyZ: 0x2c, KeyX: 0x2d, KeyC: 0x2e, KeyV: 0x2f, KeyB: 0x30, KeyN: 0x31, KeyM: 0x32,
  // 数字行
  Digit1: 0x02, Digit2: 0x03, Digit3: 0x04, Digit4: 0x05, Digit5: 0x06,
  Digit6: 0x07, Digit7: 0x08, Digit8: 0x09, Digit9: 0x0a, Digit0: 0x0b,
  // 符号
  Minus: 0x0c, Equal: 0x0d,
  BracketLeft: 0x1a, BracketRight: 0x1b, Backslash: 0x2b,
  Semicolon: 0x27, Quote: 0x28, Backquote: 0x29,
  Comma: 0x33, Period: 0x34, Slash: 0x35,
  // 修饰键（左）
  ShiftLeft: 0x2a, ShiftRight: 0x36, ControlLeft: 0x1d, AltLeft: 0x38,
  // 功能键
  F1: 0x3b, F2: 0x3c, F3: 0x3d, F4: 0x3e, F5: 0x3f, F6: 0x40,
  F7: 0x41, F8: 0x42, F9: 0x43, F10: 0x44, F11: 0x57, F12: 0x58,
  // 小键盘
  NumLock: 0x45, ScrollLock: 0x46,
  Numpad7: 0x47, Numpad8: 0x48, Numpad9: 0x49, NumpadSubtract: 0x4a,
  Numpad4: 0x4b, Numpad5: 0x4c, Numpad6: 0x4d, NumpadAdd: 0x4e,
  Numpad1: 0x4f, Numpad2: 0x50, Numpad3: 0x51, Numpad0: 0x52,
  NumpadDecimal: 0x53, NumpadMultiply: 0x37,
  // 扩展键（0xE0 前缀）
  ControlRight: 0xe01d, AltRight: 0xe038,
  NumpadEnter: 0xe01c, NumpadDivide: 0xe035,
  Insert: 0xe052, Delete: 0xe053, Home: 0xe047, End: 0xe04f,
  PageUp: 0xe049, PageDown: 0xe051,
  ArrowUp: 0xe048, ArrowLeft: 0xe04b, ArrowRight: 0xe04d, ArrowDown: 0xe050,
  MetaLeft: 0xe05b, MetaRight: 0xe05c, ContextMenu: 0xe05d,
};

export function codeToScancode(code: string): number | undefined {
  return SCANCODES[code];
}